...
- **Astro 7.x** - Fast static site generator
...
Keep the required copyright and license notices, and document significant changes as required by Apache-2.0.
...
Update Node.js to version 22.12 or higher:
...
import MyComponent from '@features/custom/components/MyComponent.astro'
...
Edit layout files in `src/app/layouts/`:
...
Edit `src/design-system/theme/tokens.ts`:
...
Configure in `src/app/config/theme.config.ts`.
...
Yes. The production build generates `/rss.xml` from published blog posts.

### Can I add search functionality?

Yes. `/search` uses a Pagefind index generated by `npm run build`; queries stay in the browser.

### Does it support comments?

Yes. Configure Giscus, Utterances, Waline, Twikoo, Artalk, or Disqus in `src/app/config/comments.config.ts`.
...
Apache License 2.0 - free to use, modify, and redistribute subject to its notice and license conditions.

### Do I need to credit the author?

Preserve the required license and copyright notices. A visible website credit is appreciated but is not a substitute for the license conditions:
